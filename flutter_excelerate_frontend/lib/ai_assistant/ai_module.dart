...
import 'context/ai_admin_context.dart';
...
export 'widgets/ai_launcher.dart';
export 'context/ai_context_provider.dart';
export 'context/ai_admin_context.dart';
...
class AiModule {
  ...
  static AiModule get instance {
    if (_instance == null) {
      throw StateError('AiModule has not been initialized. Call AiModule.initialize() first.');
    }
    return _instance!;
  }
  ...
  static void initialize({
    ...
    final studentEngine = ConversationEngine(initialConversationId: 'student_conversation');
    final adminEngine = ConversationEngine(initialConversationId: 'admin_conversation');
  ...
  void patchApplicationContext({
    ...
    AiLogger.log('Patching context', tag: 'Context', data: {
    ...
    if (screen != null && screen.screenName != current.currentScreenContext?.screenName) changed = true;
    if (system != null) changed = true;
    if (programs != null && programs != current.programsContext) changed = true;
    if (assignments != null && assignments != current.assignmentsContext) changed = true;
    if (pulse != null && pulse.lastMood != current.dailyPulseContext?.lastMood) changed = true;
    ...
    _contextManager.updateContext(current.copyWith(
      userContext: user,
  ...
  void openChat(BuildContext context, {required AiPersona persona}) {
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => AiChatPage(controller: controller, persona: persona),
      ),
    );
  }
