...
class AiRepositoryImpl {
  ...
  AiRepositoryImpl({
    ...
    required ConversationEngine conversationEngine,
  })  : _config = config,
        _client = client ?? HttpClient(),
        _contextManager = contextManager,
        conversationEngine = conversationEngine {
    AiLogger.log('Gemini client initialized with API key length: ${_config.apiKey.length}', tag: 'Init');
  }
...
final currentContext = _contextManager.currentContext;
      ...
      return AiResult.failure(e is AiException ? e : NetworkException(e.toString()));
    }
  }
...
final currentContext = _contextManager.currentContext;
      ...
      yield AiResult.failure(e is AiException ? e : NetworkException(e.toString()));
    }
  }

      Future<AiResponse> _generateResponse(Map<String, dynamic> payload) async {
    final uri = Uri.parse('${_config.apiEndpoint}/models/${_config.modelName}:generateContent');
    ...
    try {
      ...
      final httpResponse = await request.close().timeout(const Duration(seconds: 45));
      ...
      Stream<AiResponse> _generateResponseStream(Map<String, dynamic> payload) async* {
    final uri = Uri.parse('${_config.apiEndpoint}/models/${_config.modelName}:streamGenerateContent?alt=sse');
    ...
    try {
      ...
      final httpResponse = await request.close().timeout(const Duration(seconds: 45));
      if (httpResponse.statusCode != 200) {
        _handleApiError(httpResponse.statusCode, await httpResponse.transform(utf8.decoder).join());
      }

      final stream = httpResponse.transform(utf8.decoder).transform(const LineSplitter());
  ...
  void _handleApiError(int statusCode, String responseBody) {
    ...
    throw AiServiceException(message, providerName: 'gemini', statusCode: statusCode);
  }
  ...
  AiResponse _mapResponse(Map<String, dynamic> rawData) {
    ...
    final first = candidates.isNotEmpty ? candidates.first as Map<String, dynamic> : {};
