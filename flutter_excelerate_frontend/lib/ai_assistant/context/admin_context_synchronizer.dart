...
class AdminContextSynchronizer {
  static final AdminContextSynchronizer instance = AdminContextSynchronizer._internal();
  ...
  void startListening() {
    ...
    _notificationSub = NotificationService.instance.notificationsStream().listen((notifications) {
  ...
  void _sync() {
    ...
    final studentUsers = _users.where((user) => user.role.toLowerCase() != 'admin').toList();
    final inactiveCount = _users.where((user) => !user.isActive).length;

    final publishedPrograms = _programs.where((p) => p.isPublished).map((p) => p.title).toList();
    final draftPrograms = _programs.where((p) => !p.isPublished).map((p) => p.title).toList();
    final categories = _programs.map((p) => p.category).toSet().toList();
    final upcomingDeadlines = _programs.where((p) => p.applicationDeadline.isNotEmpty).map((p) => p.title).toList();
    final activeMentors = _programs.where((p) => p.mentorName.isNotEmpty).map((p) => p.mentorName).toSet().toList();
    ...
    AiModule.instance.patchApplicationContext(
      ...
      adminUsersContext: AiAdminUsersContext(
        activeStudents: studentUsers.where((u) => u.isActive).map((u) => u.email).toList(),
        inactiveStudents: studentUsers.where((u) => !u.isActive).map((u) => u.email).toList(),
        adminUsers: _users.where((u) => u.role.toLowerCase() == 'admin').map((u) => u.email).toList(),
